...
if(is_outside==true) {
  ...
  d_min2=600;
  var nearest_t;
  nearCorners=two.makeGroup();

  group.children.forEach(findMin_d);
  group.children.forEach(findMin_d2);
  
  triangle=two.makePath(newCorner.x,newCorner.y,nearCorners.children[0].x,nearCorners.children[0].y,nearCorners.children[1].x,nearCorners.children[1].y,false);
  group.add(triangle);
  ...
  function findMin_d(t) {
	...
	nearCorners.removeChildren(nearCorners.children); 
	...
	if(d_x==d_a && checkInside(t, Da)=="OUTSIDE") {nearCorners.add(new Two.Vector(t.vertices[0].x+t.position.x,t.vertices[0].y+t.position.y));}
	else if(d_x==d_b && checkInside(t, Db)=="OUTSIDE") {nearCorners.add(new Two.Vector(t.vertices[1].x+t.position.x,t.vertices[1].y+t.position.y));}
	else if(d_x==d_c && checkInside(t, Dc)=="OUTSIDE") {nearCorners.add(new Two.Vector(t.vertices[2].x+t.position.x,t.vertices[2].y+t.position.y));}
	}
    }
  }
...
function findMin_d2(t) {
  ...
  if (d_min==d_a && d_min2>Math.min(d_b,d_c)) {
    if (d_b==Math.min(d_b,d_c) && checkInside(t, Db)=="OUTSIDE")) {nearCorners.remove(nearCorners.children[0];
nearCorners.add(new Two.Vector(t.vertices[1].x+t.position.x,t.vertices[1].y+t.position.y)); d_min2=d_b;}
    else if (d_c==Math.min(d_b,d_c) && checkInside(t, Dc)=="OUTSIDE")) {nearCorners.remove(nearCorners.children[0];
nearCorners.add(new Two.Vector(t.vertices[2].x+t.position.x,t.vertices[2].y+t.position.y)); d_min2=d_c;}
  if (d_min==d_b && d_min2>Math.min(d_a,d_c)) {
    if (d_a==Math.min(d_a,d_c) && checkInside(t, Da)=="OUTSIDE")) {nearCorners.remove(nearCorners.children[0];
nearCorners.add(new Two.Vector(t.vertices[0].x+t.position.x,t.vertices[0].y+t.position.y)); d_min2=d_a;}
    else if (d_c==Math.min(d_a,d_c) && checkInside(t, Dc)=="OUTSIDE")) {nearCorners.remove(nearCorners.children[0];
nearCorners.add(new Two.Vector(t.vertices[2].x+t.position.x,t.vertices[2].y+t.position.y));  d_min2=d_c; }
  if (d_min==d_c && d_min2>Math.min(d_a,d_b)) {
    if (d_a==Math.min(d_a,d_b) && checkInside(t, Da)=="OUTSIDE")) {nearCorners.remove(nearCorners.children[0];
nearCorners.add(new Two.Vector(t.vertices[0].x+t.position.x,t.vertices[0].y+t.position.y));  d_min2=d_a;}
    else if (d_b=Math.min(d_a,d_b) && checkInside(t, Db)=="OUTSIDE")) {nearCorners.remove(nearCorners.children[0];
nearCorners.add(new Two.Vector(t.vertices[1].x+t.position.x,t.vertices[1].y+t.position.y));  d_min2=d_b;
    }
