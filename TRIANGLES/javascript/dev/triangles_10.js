  ...
  if (checkSharpness(triangle)!="sharp") {
    ...
    for (n=0; n<group.children.length; n++) {
	  ...
	  if (checkSharpness(tXAB)!="sharp" && checkSharpness(tXAC)!="sharp" && checkSharpness(tXBC)!="sharp") 	    {
	      group.add(tXAB);group.add(tXAC);group.add(tXBC);
	    }
...
function checkInsideAll_multi(s,f) {
  var result=false;
  for (n=1; n<100; n++) {
...
function checkSharpness(t) {
    ...
    /*txt.value="[0]="+t.vertices[0].x+","+t.vertices[0].y+"[1]="+t.vertices[1].x+","+t.vertices[1].y+"[2]="+t.vertices[2].x+","+t.vertices[2].y+"AB=("+AB.x+","+AB.y+")AC=("+AC.x+","+AC.y+")BC=("+BC.x+","+BC.y+")A="+A/(2*Math.PI)*360+"B="+B/(2*Math.PI)*360+"C="+C/(2*Math.PI)*360;*/
...
document.addEventListener('click', function(event) {/*
    ...
    for (n=0; n<group.children.length-1; n++) {group.children[n].fill='white';}

    convexify(counter);
    counter=counter+1;
    if (counter>=n) {counter=0;   newPoint();}*/
});
